...
def write_image(img ,imagefile, x1, y1, x2, y2, mask_status):
    if mask_status == 'with_mask':
        cv.imwrite('./train_images/with_mask/' + imagefile, img[y1:y2, x1:x2])
    elif mask_status == 'without_mask':
        cv.imwrite('./train_images/without_mask/' + imagefile, img[y1:y2, x1:x2])
    else:
        cv.imwrite('./train_images/mask_weared_incorrect/' + imagefile, img[y1:y2, x1:x2])
...
def draw(imagefile):
    ...
    with open("./json/" + jsonFileName) as json_file:
